...
void setup_game(void) {
	...
	monster_exists[0] = 1;
	door_x = room_w / 2;
	door_y = room_h / 2 + CASTLE_HEIGHT / 4;
	key_following = 0;
	player_dying = 0;
	game_score = 0;
			...
			if (!key_following) {
				...
				if (projected_x + PLAYER_WIDTH > door_x && projected_x < door_x + DOOR_WIDTH && projected_y + PLAYER_HEIGHT > door_y && projected_y < door_y + DOOR_HEIGHT) {
					if (loading_countdown == 0) {
						...
						loading_countdown = (2 / (MILLISECONDS_IN_SECOND / DELAY)) * 3 ;
					}
				}
			}
			if (shield_exists == 1) {
